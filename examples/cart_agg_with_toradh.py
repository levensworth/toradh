...
class CartService(Protocol):
    def add_item(self, cart: Cart, item: str) -> Result[Cart, InvalidItemError | DuplicateItemInCartError]: ...
...
def add_to_cart_aggregate(cart_id: int, item: str, service: CartService, repository: CartRepository) -> None:
